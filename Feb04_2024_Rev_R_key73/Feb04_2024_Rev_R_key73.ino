...
//
//  Remove:   void chasisToInsert() to simplify code.  the bracketts{} are making additions and fixes difficult  After Key #29
//
...
//        	TBlendType 	blendType = LINEARBLEND; 
  ...
  else if (  convKeyP == 70) //                     Sine_One   Cool Chase
  {
    key = keypad.getKey(); speedAndBrightnessControl();
      if ((key) && (key == '*')) {
        lcd.clear(); loop();
...
//  ******************************  Keys 71 to 79    Complex Chases ????   ********************************
  ...
  else if (  convKeyP == 73) //       Kight Rider
  {
    key = keypad.getKey(); speedAndBrightnessControl();
      if ((key) && (key == '*')) {
        lcd.clear(); loop();
  ...
  int i = beatsin8(15,0,NUM_LEDS-1);
