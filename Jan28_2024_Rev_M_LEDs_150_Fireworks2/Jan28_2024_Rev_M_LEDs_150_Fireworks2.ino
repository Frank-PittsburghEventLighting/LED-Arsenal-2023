...
//
//  Remove:   void chasisToInsert() to simplify code.  the bracketts{} are making additions and fixes difficult 
//
...
//   Fireworks-1 code:  https://www.anirama.com/1000leds/1d-fireworks/
//   
...
void setup(){
  ...
  //   initialize FastLED Settings
        FastLED.addLeds<WS2813, DATA_PIN, GRB >(leds, NUM_LEDS).setCorrection( CRGB( 255, 170, 245)) ;       //Above set to WS2813 ??????? 
...
FastLED.show();
  ...
  chasisToInsert();  //LOCATE THIS FUNCTION TO INPUT DESIRED PATTERN FROM 31-99
...
void F2shoot(byte nr) {
 ...
 void F2doSparks(byte nr){
  ...
  const int maxPos = (NUM_LEDS-1) * 128 ;
    ...
    for (int x = 0; x < F2nSparks[nr]; x++) { 
      F2sparkPos[nr][x] = F2sparkPos[nr][x] + (F2sparkVel[nr][x]>>5);   // adjust speed of sparks here
      F2sparkPos[nr][x]=constrain(F2sparkPos[nr][x],0,maxPos);
      F2sparkHeat[nr][x]=scale16(F2sparkHeat[nr][x],64000);   // adjust speed of cooldown here
